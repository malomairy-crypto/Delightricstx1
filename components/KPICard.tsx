...
export default function KPICard({ kpi, color, isInverted = false }: KPICardProps) {
  ...
  const trendBg =
    isPositive === null ? 'var(--color-surface-border)' : isPositive ? '#34d39918' : '#f8717118'
  const trendLabel =
  ...
  const atOrBeyondTarget = isInverted ? kpi.value <= kpi.target : kpi.value >= kpi.target
  ...
  return (
    <>
      <button
        ...
        className="text-left w-full flex flex-col gap-5 p-6 rounded-xl border border-surface-border bg-surface-raised hover:border-content-faint transition-colors cursor-pointer"
      >
        {/* Label + trend chip */}
        ...
        {/* Description */}
        <p className="text-xs text-content-muted leading-relaxed -mt-3">{kpi.description}</p>
        ...
        <div className="flex items-end justify-between gap-3">
          <div className="flex items-baseline gap-2">
            ...
            {kpi.unit && <span className="text-base text-content-muted">{kpi.unit}</span>}
          </div>
        ...
        <div className="space-y-2">
          <div className="h-1 rounded-full bg-surface-border overflow-hidden">
            <div
              ...
              style={{ width: `${progress}%`, backgroundColor: atOrBeyondTarget ? 'var(--color-success)' : color }}
            />
          </div>
          <div className="flex items-center justify-between">
            <span className="font-mono text-[10px] text-content-faint">{progress.toFixed(0)}% of target</span>
            <span className="font-mono text-[10px] text-content-faint">target&nbsp;{kpi.target}{kpi.unit}</span>
          </div>
        </div>
      ...
      {modalOpen && (
        <KPIModal kpi={kpi} color={color} isInverted={isInverted} onClose={() => setModalOpen(false)} />
      )}
    </>
  )
}
